...
    	
		cn.query("SELECT status_id FROM Status WHERE status_name = '" +req.params.status_name +"'", function(err, data) {
			if(err) {
				...
				cn.query("SELECT ac.action_id AS action_id, ac.flag_color_id AS flag_color_id, ac.button_label AS button_label, ac.action_duration AS action_duration, ac.status_id AS status_id, fc_flag_hex AS flag_hex FROM Actions ac, Flag_Color fc WHERE ac.flag_color_id= fc.flag_color_id AND status_id = " +data[0].status_id, function(err,data2) {
					if(err) {
						console.log(err);
						res.send(err);
					}
